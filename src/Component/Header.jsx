  ...
  return (
    <div className="relative z-10 px-20  py-8 flex items-center justify-between w-screen bg-linear-to-b from-black to-black/5">
      ...
      <div className="*:text-white font-medium mr-8 flex">
        <button className="px-8 py-1 border border-white rounded-s mx-4">
          English
        </button>
        {user && (
          <div className="flex">
            <button
              className="px-5 py-1 rounded-md bg-red-600"
              onClick={handlesignout}
            >
              Sign Out
            </button>
            <img src={user.photoURL} className="w-10 rounded-3xl"></img>
          </div>
        )}
      </div>
    </div>

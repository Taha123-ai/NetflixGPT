...
const Chatgptdetailcard = () => {
  const navigate = useNavigate();
  const { movieid } = useParams();
  const movies = useSelector((store) => store?.GPTmovies?.tmdbrecommend);
  const filtered =
  movies?.flat().find((movie) => movie.id === Number(movieid)); //find one by one
...
export default Chatgptdetailcard;